...
def read_data(config):
    ...
    if dataset == 'IAM':
        ...
        data_fol = os.path.join(data_folder_path, 'VNOnDB')
        label_paths = os.path.join(data_fol, split + '_word' + '.csv')
        all_words = []

        with open(os.path.join(data_fol, 'all_word.csv'), 'rt', encoding='utf8') as f:
        ...
        chars = np.unique(np.concatenate([[char for char in w_i.split()[-1]] for w_i in all_words]))
        char_map = {value: idx + 1 for (idx, value) in enumerate(chars)}
        char_map['<BLANK>'] = 0
        num_chars = len(char_map.keys())
        ...
        img_ids = []
        words_raw = []
        with open(os.path.join(label_paths), 'rt', encoding='utf8') as f:
            reader = csv.reader(f, delimiter='\t')
            next(reader)
            ...
            for row in reader:
                img_id = row[1]
                label = row[2]
                words_raw.append(label)
                img_ids.append(img_id)

        word_data = {}
        for img_id, label in zip(img_ids, words_raw):
            img_path = os.path.join(os.path.join(data_fol, split + '_word'), row[1] + '.png')
            img, valid_img = read_image(img_path, len(label), img_h, char_w)
            print('img_id', img_id)
            if valid_img:
                try:
